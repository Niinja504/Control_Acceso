...
export default function NewEmployeeModal({ onSaved, onClose }) {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        ...
        const filteredTeams = res.data.filter(
          (team) => !excludedIds.includes(team._id)
        );
  ...
  return (
    <div className="employee-modal-overlay">
      <form className="new-employee-form" onSubmit={handleSubmit}>
        <button
          type="button"
          className="close-employee-modal"
          onClick={onClose}
          aria-label="Cerrar"
        >
          ×
